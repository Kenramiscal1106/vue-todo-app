...
<template>
  <button v-if="!isEditFocus"
    ...
    <button class="text-secondary-accent hover:text-secondary ml-auto opacity-0 group-hover:opacity-100 transition-all"
      v-html="editIcon" @click="handleEditButton"></button>
    <button class="text-secondary-accent  hover:text-secondary opacity-0 group-hover:opacity-100 transition-all"
  ...
  <form v-else autocomplete="off" @submit.prevent="handleEditSubmit" class=' px-2 py-1 rounded-md border-2' :class="{
    ...
    <input @keydown="handleEditExit" type="text" name="title" id="" placeholder="Enter category name..."
      class="block bg-transparent outline-none text-white" v-model="titleEdit" ref="input" />
  </form>
</template>
<style scoped>
</style>