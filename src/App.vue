...
<template>
  <main class="p-4 flex flex-col items-center max-w-3xl">
    <h1>{{ time }}</h1>
    <div
      :class="'fixed top-1 left-[50%] -translate-x-[50%] bg-red-600 text-white px-4 py-2 rounded-md transition-opacity duration-300 ' + (thereIsDeadline === 0 ? ' opacity-0' : '')">
    ...
    <form @submit.prevent="addTodo" autocomplete="off" class="px-2 py-4 bg-gray-200 flex flex-col gap-2 w-full">
      <div>
    ...
    <div v-if="selectedTodo.value"
      class="flex items-center justify-center absolute top-0 left-0 w-full h-full bg-gray-600 bg-opacity-30">
    ...
    <template v-if="todoItems.value.length !== 0">
      <div class="m-2 w-full gap-1 flex flex-col items-stretch">
        <template v-for="todoItem in todoItems.value" :key="todoItem.id">
          <TodoItems v-model:todos="todoItems" v-model:selected-todo="selectedTodo" :specific-todo-item="todoItem"
